 ...
 */
void Leg::toPos(Eigen::Vector3f *pos_ptr)
{
 ...
 * @param ang
 */
void Leg::toAng(Eigen::Vector3f *ang_ptr)
{
    Eigen::Vector3f ang = *ang_ptr;
...
bool Leg::stepCycle(float direction, float stepLength, float angle)
{
    ...
    Eigen::Vector3f result = translation + rotationMatrix * functionVec;
 ...
 */
void Leg::slide(Eigen::Vector3f *targetPos_ptr)
{
    Eigen::Vector3f targetPos = *targetPos_ptr;
    for (float flag = 0; flag <= 2 * PI; flag = (float)flag + PI / 4)
    {
        Eigen::Vector3f pos1(40, 60, 60);
        toPos(&pos1);
        ...
        for (float j = PI; j > -PI; j = (float)j - 0.5)
        {
            ...
            x = -R * cosf(j);
            y = 0;
            Eigen::Vector3f pos2(tmpx + x * cosf(flag) - y * sinf(flag), tmpy + x * sinf(flag) + y * cosf(flag), tmpz);
            toPos(&pos2);
            sleep_ms(100);
        }
        sleep_ms(1000);
